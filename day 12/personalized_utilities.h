...
auto split(const std::string&& str, const std::string&& delimiter) -> std::vector<std::string> {
    ...
    auto not_empty = [](auto r) -> bool {
        return static_cast<std::size_t>(std::ranges::distance(r)) != 0;
    };

    auto range = str | 
                 std::ranges::views::split(delimiter) | 
                 std::ranges::views::filter(not_empty) | 
                 std::ranges::views::transform(to_string_l);
...
template<class T, class Fn>
constexpr void apply_on_nn_no_diags(matrix<T>&& mat, int i, int j, Fn proc) {
    std::set<std::pair<int,int>> nearest_neighbors = {
        {i + 1, j},
        {i, j + 1},
        {i - 1, j},
...
constexpr void __generate_combinations(const std::vector<char>& elems, unsigned size, std::vector<char>& curr_line, std::vector<std::string>& list) {
    for (unsigned i = 0; i < elems.size(); i++) {
        curr_line.push_back(elems[i]);
        std::string combination = "";
        if (size <= 1) { // Condition that prevents infinite loop in recursion
            for (const auto& j : curr_line)
                combination += j; // Simplified print to keep code shorter
            list.push_back(combination);
            curr_line.erase(curr_line.end() - 1);
        } else {
            __generate_combinations(elems, size - 1, curr_line, list); // Recursion happens here
            curr_line.erase(curr_line.end() - 1);
        }
    }
}

constexpr auto generate_combinations(const std::vector<char>& elems, unsigned int size) -> std::vector<std::string> {
    std::vector<std::string> combinations;
    combinations.reserve(std::pow(elems.size(),size));
    std::vector<char> line;
    __generate_combinations(elems, size, line, combinations);
    return combinations;
}