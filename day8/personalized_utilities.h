...
uint64_t count_distinct_primes(uint64_t n, const std::vector<uint64_t>& prime_divisors_table) {
    std::unordered_set<uint64_t> distinct_prime_factors = std::unordered_set<uint64_t>();
    uint64_t i = n;
    while (i > 1) {
...
constexpr uint64_t HCD(uint64_t n, uint64_t d) {
    if (d==0) { return n; }
    return HCD(d, n % d);
}
